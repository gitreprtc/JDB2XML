...
<div class="jdb2xml-tagconversion">
  ...
  <h2>CSV conversie</h2>
  <p>Upload CSV-bestanden om tags, Phoca Gallery Tags, categorieën, menu-items en artikelen om te zetten naar het XML-formaat voor import.</p>
  ...
  <div class="jdb2xml-tagconversion-section">
    ...
    <form action="index.php?option=com_jdb2xml" method="post" enctype="multipart/form-data" class="jdb2xml-tagconversion-form">
      <input type="hidden" name="task" value="csvconversionupload">
      <input type="hidden" name="conversion_type" value="menus">
      <?php echo HTMLHelper::_('form.token'); ?>

      <div class="jdb2xml-tagconversion-row">
        <label for="csv_file_menus"><strong>CSV-bestand</strong></label>
        <input type="file" name="csv_file" id="csv_file_menus" accept=".csv,text/csv">
        <button type="submit" class="btn btn-primary">Maak XML</button>
      </div>
      <p class="jdb2xml-tagconversion-status" data-file-status="csv_file_menus">
        Kies een CSV-bestand om te laden.
      </p>
    </form>
  </div>
